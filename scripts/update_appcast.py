...
Shared with AudioSwitch and ClipStack, which sign with the same EdDSA key.

Signs the archive with Sparkle's EdDSA key and rewrites appcast.xml so it contains
exactly one <item> per version (newest first). Intended to run in CI right
after a signed and notarised archive has been published to a GitHub release.

Usage:
  update_appcast.py --appcast appcast.xml --archive Candela.dmg \
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:dc="http://purl.org/dc/elements/1.1/">
  <channel>
    <title>Candela</title>
    <link>https://raw.githubusercontent.com/iamzifei/Candela/main/appcast.xml</link>
    <description>Most recent Candela updates.</description>
...
def ed_signature(sign_tool: str, key_file: str, archive: str) -> str:
    """Return the base64 EdDSA signature for the archive via sign_update -p.

    Without --key-file, sign_update reads the private key from the login
    keychain, which is where `generate_keys` puts it and how a release run on
    the maintainer's own machine signs. CI passes a key file instead, because a
    runner has no keychain to unlock.
    """
    cmd = [sign_tool, archive, "-p"]
    if key_file:
        cmd = [sign_tool, archive, "--ed-key-file", key_file, "-p"]
...
def main():
    ...
    length = os.path.getsize(args.archive)
    signature = ed_signature(args.sign_tool, args.key_file, args.archive)
