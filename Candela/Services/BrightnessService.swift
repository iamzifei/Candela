...
private func _candelaBuiltinBrightnessChanged(
    ...
    Task { @MainActor in
        ...
        NotificationCenter.default.post(name: .candelaBuiltinBrightnessDidChange, object: nil)
    }
}

extension Notification.Name {
    /// Posted when the built-in display's brightness changes (keys, ambient auto-brightness).
    static let candelaBuiltinBrightnessDidChange = Notification.Name("candela.builtinBrightnessDidChange")
    ...
    static let candelaExternalManualAdjust = Notification.Name("candela.externalManualAdjust")
...
final class BrightnessService: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.candela.brightness", qos: .userInitiated)
    ...
    private func softBrightnessKey(for displayID: CGDirectDisplayID) -> String {
        if let uuid = Self.displayUUIDString(for: displayID) {
            return "candela.softBrightness.uuid.\(uuid)"
        }
    ...
    private static func legacySoftBrightnessKey(for displayID: CGDirectDisplayID) -> String {
        "candela.softBrightness_\(displayID)"
    }
    ...
    func migrateLegacySoftBrightnessIfNeeded(for displays: [DisplayInfo]) {
        ...
        for display in displays {
            ...
            let uuidKey = "candela.softBrightness.uuid.\(display.displayUUID)"
    ...
    func refreshBrightness(for display: DisplayInfo, animated: Bool = false) async {
        ...
        // adopting that would snap the slider out of the boost region. Candela
    ...
    func startObservingBuiltinBrightness() {
        ...
        _ = register(builtin, builtin, _candelaBuiltinBrightnessChanged)
    ...
    private func noteManualBrightnessChange(displayID: CGDirectDisplayID, isBuiltin: Bool, value: Double) {
        if isBuiltin {
            NotificationCenter.default.post(name: .candelaBuiltinManualAdjust, object: nil)
        } else {
            NotificationCenter.default.post(
                name: .candelaExternalManualAdjust,
                object: nil,
