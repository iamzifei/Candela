...
final class DisplayModeController: ObservableObject {
    ...
    var resolutionGroups: [ResolutionGroup] {
    ...
    var currentGroup: ResolutionGroup? {
    ...
    // Internal rather than fileprivate: the blocks that call it are in
    // DisplayModeListView now.
    func selectResolution(_ group: ResolutionGroup) {
...
/// Internal, not private: the controller's `currentGroup` returns one, and the
/// resolution blocks live in another file now.
