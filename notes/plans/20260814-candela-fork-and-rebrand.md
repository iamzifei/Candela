...
- [x] **辅助功能设置下逐一截图（4 项中的 3 项，2026-08-16）** —— 截图在 `notes/a11y/`
  ...
  **我之前说「做不到」是错的。** `defaults write com.apple.universalaccess` 确实被 TCC 挡住，
  ...
  | 减弱动态效果 | ✅ 渲染正确（`notes/a11y/motion.webp`）。开关在**辅助功能 → Motion** 分组，不在 Display——我之前一直在 Display 里找所以找不到 |

  **「动画被抑制」这一点没有验到，说清楚边界**：我测了面板打开时高度随时间的变化，
  开与关都只有单一高度值——**说明打开本身不带高度动画**，Reduce Motion 在这一刻没有东西可抑制。
  真正的高度动画发生在页面切换与分区展开时，而那需要在面板内部点击，合成事件进不去。
  代码路径读过是对的（弹簧直接落位、SwiftUI 动画 0 时长），但那是读代码，不是实测。
